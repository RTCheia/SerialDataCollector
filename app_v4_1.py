    ...
    def _run(self):
        ...
        with open(self.output_file, "w", encoding="utf-8") as out_f:
            ...
            while self.is_running:
                ...
                curr_t = time.time()
                # 操作系统级高效批量缓冲 flush（不阻塞物理磁盘总线，防意外丢失）
            ...
            while self.is_running:
                try:
                    ...
                    while len(buffer) >= self.frame_len:
                        ...
                        curr_t = time.time()
                        # 操作系统级高效批量 flush：每 50 帧或 0.2 秒提交给 OS PageCache，彻底释放物理 I/O
                        if unflushed_count >= 50 or (curr_t - last_flush_time >= 0.2):
                            out_f.flush()
                            unflushed_count = 0
                            last_flush_time = curr_t
...
class AppV4:
    def __init__(self, root):
        self.root = root
        self.root.title("多串口数据采集分析系统 V4.1 (高性能低I/O版)")
    ...
    def _create_widgets(self):
        ...
        # 高性能提示
        tk.Label(top_bar, text="⚡ OS 高速缓冲落盘 (零卡顿/零丢包) | 🎯 预览锁定 10 帧/秒", fg="#0284C7", bg="#FFFFFF", font=("Microsoft YaHei", 9, "bold")).pack(side="left", padx=15)
